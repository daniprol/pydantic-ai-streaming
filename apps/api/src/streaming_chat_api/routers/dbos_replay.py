...
from streaming_chat_api.services import dbos_replay as replay_service
from streaming_chat_api.ui import replay_stream_response


router = APIRouter(prefix='/flows/dbos-replay', tags=['dbos-replay'])
...
async def list_conversations(
    ...
    return await replay_service.list_conversations(session, pagination)
...
async def create_conversation(session: SessionDep) -> ConversationCreateResponse:
    return await replay_service.create_conversation(session)


@router.delete('/conversations/{conversation_id}', status_code=204)
async def delete_conversation(conversation_id: UUID, session: SessionDep) -> None:
    deleted = await replay_service.delete_conversation(session, conversation_id)
    if not deleted:
...
async def get_messages(
    ...
    return await replay_service.get_messages(session, conversation_id)
...
async def chat(
    ...
    return await replay_service.stream_chat(
        session=session,
        request=request,
        resources=resources,
