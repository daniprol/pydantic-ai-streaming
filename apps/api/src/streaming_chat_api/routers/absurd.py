...
from streaming_chat_api.services import absurd as absurd_service


router = APIRouter(prefix='/flows/absurd', tags=['absurd'])
...
async def list_conversations(
    ...
    return await absurd_service.list_conversations(session, pagination)
...
async def create_conversation(session: SessionDep) -> ConversationCreateResponse:
    return await absurd_service.create_conversation(session)


@router.delete('/conversations/{conversation_id}', status_code=204)
async def delete_conversation(conversation_id: UUID, session: SessionDep) -> None:
    deleted = await absurd_service.delete_conversation(session, conversation_id)
    if not deleted:
...
async def get_messages(
    ...
    return await absurd_service.get_messages(session, conversation_id)
...
async def chat(
    ...
    return await absurd_service.stream_chat(
        session=session,
        request=request,
        resources=resources,
